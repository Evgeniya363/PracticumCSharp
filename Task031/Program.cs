...
int[] InitArray(int left, int right)
{
  ...
  int[] array = new int[n]; Console.Write($"{array[0]} ");
...
// для int sign - знак чисел, которые мы будем суммировать 
// (-1 -отрицательные, 1 - положительные) 
int Summ(int sign, int[] a)
{
  ...
  for (int i = 0; i < a.Length; i++)
  {
    ...
    // переменная summ наращивается на величину a[i].
  ...
  // summ, управление возвращается в строку вызова метода (22б 23)
  return summ;
}

// Пользовательский метод PrintArray принимает 1 параметра (аргумент) - 
